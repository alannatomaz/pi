...
int main(int argc, char *argv[])
{
    ...
    printf("==========================================\n");
    /* ========================================================= */
    /*   MEDICAO DE TEMPO - TABELA HASH                          */
    /*   (mesmo protocolo da busca sequencial, mesmas amostras)  */
    /* ========================================================= */
    double tempo_rodada_h[N_RODADAS];
    double media_inicio_h[N_RODADAS];
    double media_meio_h[N_RODADAS];
    double media_final_h[N_RODADAS];
    double media_inexistente_h[N_RODADAS];

    for (r = 0; r < N_RODADAS; r++)
    {
        printf("\n==========================================\n");
        printf("  Rodada %d (HASH)\n", r + 1);
        printf("==========================================\n");
        ...
        t_ini = clock();
        for (i = 0; i < N_BUSCAS_POR_BLOCO; i++)
            hash_buscar(tabela, m, ids_teste[i]);
        t_fim = clock();
        t_bloco = (double)(t_fim - t_ini) / CLOCKS_PER_SEC;
        media_inicio_h[r] = t_bloco / N_BUSCAS_POR_BLOCO;
        printf("  Inicio      : %d buscas | Media: %.8f s/busca\n", N_BUSCAS_POR_BLOCO, media_inicio_h[r]);
        ...
        t_fim = clock();
        t_bloco = (double)(t_fim - t_ini) / CLOCKS_PER_SEC;
        media_meio_h[r] = t_bloco / N_BUSCAS_POR_BLOCO;
        printf("  Meio        : %d buscas | Media: %.8f s/busca\n", N_BUSCAS_POR_BLOCO, media_meio_h[r]);
        ...
        t_fim = clock();
        t_bloco = (double)(t_fim - t_ini) / CLOCKS_PER_SEC;
        media_final_h[r] = t_bloco / N_BUSCAS_POR_BLOCO;
        printf("  Final       : %d buscas | Media: %.8f s/busca\n", N_BUSCAS_POR_BLOCO, media_final_h[r]);
        ...
        t_fim = clock();
        t_bloco = (double)(t_fim - t_ini) / CLOCKS_PER_SEC;
        media_inexistente_h[r] = t_bloco / N_BUSCAS_POR_BLOCO;
        printf("  Inexistente : %d buscas | Media: %.8f s/busca\n", N_BUSCAS_POR_BLOCO, media_inexistente_h[r]);

        tempo_rodada_h[r] = (media_inicio_h[r] + media_meio_h[r] + media_final_h[r] + media_inexistente_h[r]) * N_BUSCAS_POR_BLOCO;
        printf("------------------------------------------\n");
        printf("  Tempo total da rodada : %.6f s\n", tempo_rodada_h[r]);
        printf("  Media por busca       : %.8f s\n", tempo_rodada_h[r] / (N_BUSCAS_POR_BLOCO * 4));
    }

    /* Resumo final HASH */
    double soma_rodadas_h = 0;
    for (r = 0; r < N_RODADAS; r++)
        soma_rodadas_h += tempo_rodada_h[r];

    printf("\n==========================================\n");
    printf("  Resumo Final - TABELA HASH\n");
    printf("==========================================\n");
    for (r = 0; r < N_RODADAS; r++)
        printf("  Media de tempo - Rodada %d : %.8f s/busca\n", r + 1, tempo_rodada_h[r] / (N_BUSCAS_POR_BLOCO * 4));
    double media_geral_hash = (soma_rodadas_h / N_RODADAS) / (N_BUSCAS_POR_BLOCO * 4);
    printf("  Media geral               : %.8f s/busca\n", media_geral_hash);
    printf("  Tempo total das execucoes : %.6f s\n", soma_rodadas_h);
